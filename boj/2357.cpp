...
int construct2(int node, int l, int r) {
    if (l == r) return segtree2[node] = d[l];
    int mid = (l + r) / 2;
    int maxl = construct2(node*2, l, mid);
    int maxr = construct2(node*2+1, mid+1, r);
    return segtree2[node] = max(maxl, maxr);
}
...
int query2(int node, int l, int r, int nodel, int noder) {
    if (r < nodel || noder < l) return INT_MIN;
    if (l <= nodel && noder <= r) return segtree2[node];
    int mid = (nodel + noder) / 2;
    return max(query2(node*2, l, r, nodel, mid)
            , query2(node*2+1, l, r, mid+1, noder));
}
...
int main() {
    ...
    while (m--) {
        int a, b; cin >> a >> b; a--; b--;
        cout << query(1, a, b, 0, n-1) << " " << query2(1, a, b, 0, n-1) << "\n";
    }
    return 0;
}
